...
export default function LawLibrarySection() {
  const { LegalData } = useSelector((state) => state.LegalRTK);
  const [activeCategory, setActiveCategory] = useState("القانون المدني");
  ...
  const categoryFiltered =
    LegalData?.data?.filter((law) => law.name === activeCategory) || [];
    ...
    <>
      ...
      <section className="bg-black px-4 py-20 text-white md:px-10">
        <div className="mx-auto max-w-6xl">
          ...
          >
            <h1 className="text-4xl font-semibold md:text-5xl">Law Library</h1>

            <p className="mt-3 text-sm text-gray-500">
              Home / <span className="text-yellow-400">Law Library</span>
            </p>
          </motion.div>
          ...
          >
            <div className="space-y-16">
              {(search.trim() ? searchResults : categoryFiltered).map(
                (law, index) => (
                  <motion.div
                    key={law.id}
