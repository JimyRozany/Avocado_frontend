...
import { GetWarning } from "@/lib/LawyerManagement";
...
const { UserData } = useSelector((state) => state.UserRTK);
  ...
  return (
    <div className=" font-['Instrument_Sans',sans-serif] pb-15">
      {/* Top Bar */}
          ...
          <AnimatePresence mode="wait">
            {!isEditing ? (
              <motion.button
                ...
                onClick={handleEdit}
                className="flex items-center cursor-pointer gap-2 px-5 py-2.5 bg-[#1a1a1a] text-white text-sm font-semibold rounded-lg hover:bg-[#2d2d2d] transition-colors"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
