...
const statusConfig = {
  Resolved: { color: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-200", dot: "bg-emerald-500" },
  Active: { color: "text-amber-600", bg: "bg-amber-50", border: "border-amber-200", dot: "bg-amber-500" },
...
export default function WarningsHistoryCard({ warnings = [] }) {
  ...
  const handleStatusChange = (index, newStatus) => {
    setLocalWarnings((prev) => prev.map((w, i) => i === index ? { ...w, status: newStatus } : w));
  };
  ...
  return (
    <motion.div layout className="bg-white rounded-2xl border border-stone-200 p-6 shadow-sm">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h2 className="text-lg font-bold text-stone-800 tracking-tight">Warnings History</h2>
          <p className="text-xs text-stone-400 mt-0.5">{localWarnings.length} warnings on record</p>
        </div>
        ...
        <table className="w-full min-w-[520px] text-sm">
          <thead>
            <tr className="border-b border-stone-100">
              {["Warning ID", "Reason", "Sent By", "Date", "Status"].map((h) => (
                <th key={h} className="text-left py-2 px-3 text-[11px] font-semibold uppercase tracking-wider text-stone-400">
                  {h}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {localWarnings.map((w, i) => (
              ...
              >
                <td className="py-3 px-3 font-mono text-xs font-semibold text-stone-600">{w.id}</td>
                <td className="py-3 px-3 text-stone-500 max-w-[120px]">
                  <span className="block truncate text-xs">{w.reason}</span>
                </td>
                <td className="py-3 px-3 text-xs text-stone-500">{w.sentBy}</td>
                <td className="py-3 px-3 text-xs text-stone-400 whitespace-nowrap">{w.date}</td>
                <td className="py-3 px-3">
                  <StatusBadge
                    status={w.status}
                    isEditing={true}
                    onChange={(s) => handleStatusChange(i, s)}
                  />
                </td>
              </motion.tr>
            ))}
