...
const InputField = ({ label, value, isEditing, onChange, className = "", textarea = false }) => {
  return (
...
export default function LawyerSummaryCard({ data, isEditing, onChange, onImageChange }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-4 mb-6">
        ...
        <div>
          <h2 className="text-lg font-bold text-stone-800 tracking-tight">Lawyer Summary</h2>
          <p className="text-xs text-stone-400 mt-0.5">Personal & professional info</p>
          {isEditing && (
            <button
              onClick={handleImageClick}
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="relative">
            <div className={`w-full px-3 py-2.5 rounded-lg border min-h-10.5 text-sm flex items-center gap-2 ${isEditing ? "border-stone-200 bg-stone-50" : "border-stone-200 bg-white"}`}>
              <div className={`w-2 h-2 rounded-full ${data.summary?.accountStatus === "Active" ? "bg-emerald-500" : "bg-rose-500"}`} />
              <span className={`font-medium text-sm ${data.summary?.accountStatus === "Active" ? "text-emerald-700" : "text-rose-700"}`}>
                {data.summary?.accountStatus || "Active"}
              </span>
            </div>
            <label className="absolute -top-2 left-2.5 text-[10px] font-semibold uppercase tracking-wider text-stone-400 bg-white px-1">
              Account Status
